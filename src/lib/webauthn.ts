...
interface PublicKeyObjectResult {
  rpIdHash: Buffer;
  flags: number;
  signCount: number;
  aaguid: Buffer;
  ...
  theRest: Buffer;
  publicKeyObject: Map<string, Uint8Array>;
}

function getPublicKeyObject(attestationObject: string): PublicKeyObjectResult {
  const decodedData = CBOR.decode(
    base64url.toBuffer(attestationObject),
  ) as unknown as { authData: Buffer };
  const authData = decodedData.authData;
  ...
  if (flags & 0x40) {
    ...
    const decodedCredentialPublicKey = CBOR.decode(
      credentialPublicKey,
    ) as Record<string, Uint8Array>;
    const publicKeyObject = new Map<string, Uint8Array>(
      Object.entries(decodedCredentialPublicKey),
    );
